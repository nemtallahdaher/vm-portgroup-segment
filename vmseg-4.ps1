...
if ($dcselected -and $clusterselected) {
	...
	import-csv $csvfile | % {
		...
		$myvms=get-view -viewtype network -filter @{"name" = "^"+$thisportgroup+"$"}
        if ($myvms.vm -ne $NULL) {
		    $vms=get-vm -id $myvms.vm|Select Name, @{N="Cluster";E={Get-Cluster -VM $_}} | where {$_.cluster.name -eq $clusterselected}
		    foreach($vmview in $vms )
		    {
			    write-Host -NoNewline -BackgroundColor blue  $vmview.cluster.name, $vmview.name " "
			    $doit=0
			    if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
				    $doit=1
				    get-networkadapter -vm $vmview.name | where {$_.networkname -eq $thisportgroup}|set-networkadapter -NetworkName $_.Segment -confirm:$false
			    } 
			    else {
				    get-networkadapter -vm $vmview.name | where {$_.networkname -eq $thisportgroup}|set-networkadapter -NetworkName $_.Segment -whatif -confirm:$false
			    }
		    }
        }
	}
